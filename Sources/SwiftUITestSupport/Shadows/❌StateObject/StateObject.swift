...
@MainActor @propertyWrapper @preconcurrency public struct StateObject<ObjectType> : DynamicProperty where ObjectType : ObservableObject {

  private var storage: SwiftUI.StateObject<ObjectType>

  private enum ThunkObject {
    case unDispatched(() -> ObjectType)
    case loaded(ObjectType)

    var object: ObjectType {
      mutating get {
        switch self {
        case let .unDispatched(thunk):
          let object = thunk()
          self = .loaded(object)
          return object
  ...
  @ThreadSafe
  private var testValue: ThunkObject
